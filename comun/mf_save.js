...
var MF = (function(){
  ...
  function cargaPaletas(nombreModulo, alListo){
    ...
    Promise.reject(0)
      ...
      .catch(function(){ /* pas de table : on garde le thème du CSS */ });
  }
  ...
  function ponFuente(){
    if(!datos) return;
    /* v2 (P-H, 08/09) : l'infobulle du 📂 dit QUEL dossier est chargé.
       Avant : « Abrir un expediente ». Après : le nom du fichier. */
    var b = document.getElementById('mf-abrir');
    if(b) b.title = (nombre||'') + (p_carpeta? '  ·  '+p_carpeta : '')
  ...
  function abeja(destino){
    if(enMarco) return;   /* le cadre du lanceur porte déjà ce bouton */
    var d = destino || params().panal || '../index.html';
    var b=document.createElement('div');
    b.id='mf-abeja';
    b.innerHTML='🐝';
  ...
  return { arranca:arranca, carga:carga, params:params, abeja:abeja, abrirFichero:abrirFichero,
           ...
           ponVersion:ponVersion, tip:tip, config:config, cargaConfig:cargaConfig,
           ponFuente:ponFuente,
